...
(function () {
  ...
  // Signaling. The desktop build uses Kodub's servers (the game's own API client). A web build served from
  // another domain cannot: Kodub's API only answers their own origins. tweaks/collab-config.js can point at
  // a self-hosted signaling server instead, which speaks the same message shapes.
  const DEFAULT_ICE = [{ urls: ['stun:stun.l.google.com:19302'] }, { urls: ['stun:stun.cloudflare.com:3478'] }];
  const config = () => (window.PTCollabConfig && typeof window.PTCollabConfig === 'object' ? window.PTCollabConfig : {});
  function ownSignaling() {
    const s = config().signaling;
    return typeof s === 'string' && /^wss?:\/\/\S+$/.test(s) ? s.replace(/\/+$/, '') : null;
  }
  function hostSocket() {
    const s = ownSignaling();
    return s ? new WebSocket(s + '/host') : api().createMultiplayerHostWebSocket();
  }
  function joinSocket() {
    const s = ownSignaling();
    return s ? new WebSocket(s + '/join') : api().createMultiplayerJoinWebSocket();
  }
  async function iceList() {
    if (!ownSignaling()) return sanitizeIce(await api().getIceServers());
    const own = sanitizeIce(config().iceServers);
    return own.length ? own : DEFAULT_ICE.slice();
  }
  ...
  class HostNet {
    constructor(ctrl) {
      ...
      this.ws = null; this.key = null; this.keepalive = null; this.expiry = null; this.iceFallback = [];
    ...
    openInvite(fresh) {
      ...
      this.ctrl.renderUI();
      iceList().then((list) => { this.iceFallback = list; }).catch(() => {});
      let ws;
      try { ws = hostSocket(); }
    ...
    handleJoin(o, send) {
      ...
      const joinerIce = sanitizeIce(o.iceServers);
      const pc = new RTCPeerConnection({ iceServers: joinerIce.length ? joinerIce : this.iceFallback });
  ...
  class ClientNet {
    ...
    async join(code) {
      let iceServers;
      try { iceServers = await iceList(); } catch { throw new Error('server'); }
      ...
      await new Promise((resolve, reject) => {
        ...
        let ws;
        try { ws = joinSocket(); } catch { return finish(new Error('server')); }
        this.ws = ws;
        (async () => {
          const offer = await pc.createOffer();
          await pc.setLocalDescription(offer);
          const sendJoin = () => {
            const join = { version: GAME_VERSION, inviteCode: code, offer: offer.sdp, mods: [Core.MOD_ID], isModsVanillaCompatible: false,
              nickname: profile.nickname, countryCode: profile.countryCode, carStyle: profile.carStyle };
            if (ownSignaling()) join.iceServers = iceServers; // Kodub's server supplies these itself
...
`;
  ...
  const PTCollab = {
    ...
    /** A web build with no signaling server of its own cannot reach Kodub's: say so plainly. */
    noSignaling() {
      if (ownSignaling() || window.electron) return false;
      this.status = 'Collaborating is not set up on this site yet: it needs a signaling server.';
    ...
    startHost() {
      if (this.session || !this.h || this.noSignaling()) return;
      this.flushLocal();
    ...
    joinSession(code) {
      if (this.session || this.joining || !this.h || this.noSignaling()) return;
